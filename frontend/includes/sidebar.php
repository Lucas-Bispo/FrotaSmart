<aside class="w-full lg:w-64 bg-slate-900 text-white lg:h-screen lg:fixed left-0 top-0 overflow-y-auto shadow-xl">
    ...
    <nav class="mt-2 p-3 space-y-1">
        ...
        <?php endif; ?>
        <?php if (user_can(\FrotaSmart\Application\Security\Rbac::PERMISSION_FLEET_READ)): ?>
            <a href="/manutencoes.php" class="block rounded-xl py-3 px-4 hover:bg-slate-800 <?php echo basename($_SERVER['PHP_SELF']) === 'manutencoes.php' ? 'bg-slate-800 border border-slate-700' : ''; ?>">
                Manutencoes
            </a>
        <?php endif; ?>
        <?php if (user_can(\FrotaSmart\Application\Security\Rbac::PERMISSION_FLEET_READ)): ?>
            <a href="/abastecimentos.php" class="block rounded-xl py-3 px-4 hover:bg-slate-800 <?php echo basename($_SERVER['PHP_SELF']) === 'abastecimentos.php' ? 'bg-slate-800 border border-slate-700' : ''; ?>">
                Abastecimentos
            </a>
        <?php endif; ?>
        <?php if (user_can(\FrotaSmart\Application\Security\Rbac::PERMISSION_FLEET_READ)): ?>
            <a href="/viagens.php" class="block rounded-xl py-3 px-4 hover:bg-slate-800 <?php echo basename($_SERVER['PHP_SELF']) === 'viagens.php' ? 'bg-slate-800 border border-slate-700' : ''; ?>">
                Viagens
            </a>
        <?php endif; ?>
        <?php if (user_can(\FrotaSmart\Application\Security\Rbac::PERMISSION_FLEET_READ)): ?>
        ...
        <?php endif; ?>
        <?php if (user_can(\FrotaSmart\Application\Security\Rbac::PERMISSION_FLEET_READ)): ?>
            <a href="/relatorios.php" class="block rounded-xl py-3 px-4 hover:bg-slate-800 <?php echo basename($_SERVER['PHP_SELF']) === 'relatorios.php' ? 'bg-slate-800 border border-slate-700' : ''; ?>">
                Relatorios
            </a>
        <?php endif; ?>
